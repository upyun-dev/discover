...
var Table = new Class({/*{{{*/
    ...
    findById: function(id, callback) {
        ...
        if (Array.isArray(id) && this.pks.length === id.length) {
            ...
            args = this.pks.map(function(f){ return id[f.name]; })
    ...
    update: function(model, callback) {
        ...
        var sql = 'UPDATE `' + this.name + '` SET ' + util.map(args, function(v, k){return '`' + k + '` = ?';}).join(', ');
        var condition = this.pks.map(function(f){ return '`' + f.name + '` = ?'; }).join(' AND ');
        sql += ' WHERE ' + condition;
    ...
    _sqlLoad: function(){
        if (!this._sqls['load']) {
            var cols = this.allFields.map(function(f){ return '`' + f.name + '`'; }).join(', ');
            var condition = this.pks.map(function(f){ return '`' + f.name + '` = ?'; }).join(' AND ');
    ...
    _sqlInsert: function(){
        if (!this._sqls['insert']) {
            var cols = this.nonAutoFields.map(function(f){ return '`' + f.name + '`'; }).join(', ');
...
Criteria.Select = new Class({
    ...
    toSQL: function() {
        ...
        var cols = table.pks.map(function(f){ return '`' + f.name + '`'; }).join(', ');
