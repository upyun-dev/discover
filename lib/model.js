...
function Model(options, params) {
  // 生成一份浅拷贝, 防止 Model 内修改原始 params
  ...
  var table = tables[tableName] = new Table(tableName, fields, options.db);
  ...
  util.inherits(newModel, EventEmitter2);
  ...
  newModel.db = options.db;
  newModel.cache = options.cache;
  newModel.Q = options.Q;
...
exports.init = function(options) {
  // Model.db = options.db;
  // Model.cache = options.cache;
  // Model.Q = Criteria.init({ db: Model.db, cache: Model.cache });
  ...
  //   Model.cache = options.cache;
  // }
  return Model.bind(null, {
    db: options.db,
    cache: options.cache,
    Q: Criteria.init({ db: options.db, cache: options.cache })
  });
};
