...
function Model(params) {
  ...
  // monitor auto-executed method
  // Node.js v4.x
  Object.observe(proto, function(changes) {
    changes.forEach(function(change) {
      if (!change.name.match(/^validate/))
        return;

      if (!newModel._prefixMethod.validate)
        newModel._prefixMethod.validate = [];

      switch (change.type) {
        case 'add':
          newModel._prefixMethod.validate.push(change.name);
          break;
        case 'update':
          if (typeof change.object[change.name] !== 'function')
            change.type = 'delete';
          else
            break;
        case 'delete':
          var idx = newModel._prefixMethod.validate.indexOf(change.name);
          newModel._prefixMethod.validate.splice(idx, 1);
      }
    });
  });
