...
exports.classMethods = {
  ...
  find: function(query, opts, callback) {
    ...
    opts = Object.assign({
      // 兼容旧的约束
      orderBy: this.$table.fields.id ? { 'id': 'desc' } : null,
      json: false,
      limit: 20,
      page: 1
    }, opts);
  ...
  update: function(model, callback) {
    ...
    var _update = function(done) {
      self.$table.update(model, function(err, updated) {
        if (updated)
          model._oldStates = model.toJSON(true);

        if (err) {
          done(err, updated);
        } else if (updated) {
          self.cache.del(self._cacheKey(model), function(err) {
            done(null, oldModel, model);
          });
        } else {
          done(null, oldModel, model);
        }
...
exports.instanceMethods = {
  $initialize: function(attributes, options) {
    ...
    this._oldStates = this.toJSON(true);
  ...
  toJSON: function(secure) {
    let jsonobj = util.clone(this.attributes);
    let fields = this.$model.$table.fields;
    if (!secure)
      Object.keys(fields).forEach((name) => { if (fields[name].secure) delete jsonobj[name]; });
