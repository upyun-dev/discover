...
exports.classMethods = {
  ...
  insert: function(model, callback) {
    ...
    // auto-execute validations
    model._walk('validate');

    // perform an opts before insert
    self.emit('HOOK-BEFORE-INSERT', model);
    ...
    this.$table.insert(model, function(err, result) {
      ...
      cache.del(self._cacheKey(result), function(err, done) {
        ...
        self.emit('HOOK-AFTER-INSERT', model);
      });
    });
  ...
  update: function(model, callback) {
    ...
    self.emit('HOOK-BEFORE-UPDATE', model);
    ...
    this.$table.update(model, function(err, updated) {
      ...
      if (updated) {
        cache.del(self._cacheKey(model), function(err, done) {
          ...
          self.emit('HOOK-AFTER-UPDATE', model);
        });
      } else {
        if (callback) callback(null, model);
        // perform an opts after update
  ...
  delete: function(model, callback) {
    ...
    // self.emit('HOOK-BEFORE-DELETE', model);
    ...
    this.$table.delete(model, function(err, deleted) {
      ...
      if (deleted) {
        cache.del(self._cacheKey(model), function(err, done) {
          ...
          // self.emit('HOOK-AFTER-DELETE', model);
        });
      } else {
        if (callback) callback(null, model);
        // // perform an opts after delete
  ...
  before: function(model, methodName, exec) {
    if (typeof model[methodName] === 'function')
      model.on(`HOOK-BEFORE-${methodName}`, exec);
  },

  after: function(model, methodName, exec) {
    if (typeof model[methodName] === 'function')
      model.on(`HOOK-AFTER-${methodName}`, exec);
  },
...
exports.instanceMethods = {
  ...
  _performValidation: function(attrs, options) {
    var error = (function() {
      this.validate(attrs) ? null : new Error('invalid');
    })();
  ...
  _walk: function(prefix) {
    ...
    model.invalid = Object
      ...
      .every(function(validateMethod) {
        self[validateMethod](self.attributes);
      });
  },
  ...
  //   return this.$model.validate(this, logic);
  // }
};
