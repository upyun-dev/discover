...
exports.classMethods = {
  ...
  count: function(opts, callback) {
    ...
    _.forEach(this.$table.fields, function(field, key) {
      if (field.pk || field.unique) return;

      var opt = camelCase(field.name);
      if (opts[opt] && typeof opts[opt] === 'object') {
        q.push(Q[opts[opt].op](field.name, opts[opt].value));
      }
      else if (opts[opt]) {
        q.push(Q.eq(field.name, opts[opt]));
      }
    });

    Q.count(this)
     .where(q.length ? Q.and(q) : null)
     .execute(callback);
  },
  ...
  findOne: function(opts, callback) {
    ...
    this.find(opts, function(err, rows) {
  ...
  findWithCount: function(opts, callback) {
    var self = this;
    async.parallel({
      rows: function(callback) {
        self.find(opts, callback);
      },
  ...
  // exec-serise
  _walk: function(model, prefix) {
    var pattern = new RegExp(`^${prefix}`);

    var ret = Object
      .keys(Object.getPrototypeOf(model))
  ...
  insert: function(model, callback) {
    ...
    var validationTasks = self._walk(model, 'validate');
    ...
    var _insert = function(done) {
      self.$table.insert(model, function(err, result) {
        if (err) {
          ...
          cache.del(self._cacheKey(result), function(err) {
            done(null, result);
          });
        }
      });
    ...
    var _update = function(done) {
      self.$table.update(model, function(err, updated) {
        if (err) {
          ...
          cache.del(self._cacheKey(model), function(err) {
            done(null, model);
          });
        } else {
          done(null, model);
...
exports.instanceMethods = {
  ...
  // hooks
  before: function(methodName, exec) {
    this.$model.before(methodName, exec);
  },

  after: function(methodName, exec) {
    this.$model.after(methodName, exec);
  },
