...
var criteriaFilterNotInstMethods = {
  $operator: 'NOT',

  toSQL: function() {
    return this.$operator + ' ' + this.filters.map(function(f) { return f.toSQL(); }).join(' ') + '';
  }
};
...
var criteriaClassMethods = {
  ...
  xor: function() {
    return new this.Filter.XOR(util.toArray(arguments));
  },

  not: function() {
    return new this.Filter.NOT(util.toArray(arguments));
  },
...
function init(options) {
  ...
  Criteria.Filter.XOR = new Class({
    Extends: Criteria.Filter.AND,
    $operator: 'XOR'
  });
  criteriaFilterNotInstMethods.Extends = Criteria.Filter.AND;
  Criteria.Filter.NOT = new Class(criteriaFilterNotInstMethods);
